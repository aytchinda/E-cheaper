...
<div class="d-flex justify-content-end">
    ...
    <a href="{{ route('admin.carrier.create') }}" class="btn btn-success m-1">
            ...
            <tbody>
                @foreach($carriers as $carrier)
						<tr><td>{{ $carrier->id }}</td>
							...
							<td>{{ number_format($carrier->price, 2, ',', ' ') . ' €' }}</td>
						<td>
                    <a href="{{ route('admin.carrier.show', ['id' => $carrier->id]) }}" class="btn btn-primary btn-sm">
                        <i class="fa-solid fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.carrier.edit', ['id' => $carrier->id]) }}" class="btn btn-success btn-sm">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </a>
                    <a href="#" data-id="{{ $carrier->id }}" class="btn btn-danger btn-sm deleteBtn">
...
@section('scripts')
    ...
    <script>
        ...
        checkbox.onchange = async (event) => {
            ...
            const response = await fetch('/admin/carriers/speed/' + id, {
                method: 'PUT',
        ...
        deleteButtons.forEach(deleteButton => {
            deleteButton.addEventListener('click', (event)=>{
                ...
                confirmDeleteBtn.addEventListener('click',async ()=>{
                    const csrfToken = document.head.querySelector('meta[name="csrf-token"]').content;
                    const response = await fetch('/admin/carriers/delete/'+id , {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': csrfToken
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const tableHeaders = document.querySelectorAll('#Carrier th');
            ...
            tableHeaders.forEach(function(header, index) {
                ...
                const savedSelection = localStorage.getItem('selectedColumns#Carrier');
        ...
        function toggleColumn(columnIndex, show) {
            const dataTable = document.getElementById('Carrier');
        ...
        function saveSelection() {
            ...
            localStorage.setItem('selectedColumns#Carrier', JSON.stringify(selectedColumns));
        }
        ...
        function sortTable(columnIndex) {
            const table = document.getElementById('Carrier');

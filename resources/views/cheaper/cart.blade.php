...
<div class="container my-5">
    ...
    @if (session()->has('cart') && isset(session()->get('cart')['items']) && count(session()->get('cart')['items']) > 0)
                ...
                <tbody class="table-group-divider">
                    @foreach (session()->get('cart')['items'] as $item)
                        <tr data-product-id="{{ $item['product']['id'] }}">
                            ...
                            <td>
                                <form action="{{ route('removeFromCart', ['productId' => $item['product']['id'], 'quantity' => $item['quantity']]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="d-flex align-items-center my-4">
            ...
            <div>
                <form action="{{ route('cart.index') }}" method="GET" class="d-flex align-items-center">
                    <select class="form-control form-select-sm w-auto" name="carrier_id" id="carrier_id" onchange="this.form.submit()">
        ...
        <div class="text-end my-4">
            ...
            <h4>Shipping:
                <span id="carrier-name">{{ $selectedCarrier ? $selectedCarrier->name : 'Sélectionnez un transporteur' }}</span>
                - <span id="shipping-price">€{{ $selectedCarrier ? number_format($selectedCarrier->price, 2, ',', ' ') : '' }}</span>
            </h4>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateTotalPrice() {
            ...
            const shippingPrice = parseFloat(shippingPriceElement.textContent.replace('€', '').replace(',', '.')) || 0;
